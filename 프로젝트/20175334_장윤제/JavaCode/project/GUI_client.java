...
public class GUI_client extends JFrame implements CoapClient{
	...
	public void onResponse(CoapClientChannel channel, CoapResponse response) {
		...
		//LED 제어(observe의 응답인지 확인 --> illum 값 확인 --> illum값에 따라 led resource에 대한 PUT 요청 메시지 전송)
		//옵저브의 응답인지 확인
		if(Encoder.ByteToString(response.getToken()).equals("obTocken")) {
			String getPayload = Encoder.ByteToString(response.getPayload());
			if(getPayload.indexOf("PR")!=-1) {  //받은 payload값이 조도값이라면
				int illum = Integer.parseInt(getPayload.substring(2));
				this.control_led(illum);
			}
			else if(getPayload.indexOf("Tp")!=-1) {  //받은 payload값이 온도값이라면
				int temp = Integer.parseInt(getPayload.substring(2));
				this.control_motor(temp);
			}
			
		}
	}
	//조도값에 따른 조명제어
	public void control_led(int illum) {
		if (illum <= 15) {
			//on
	...
	//온도값에 따른 모터제어
	public void control_motor(int temp) {
		if (temp >= 42) {
			//on
			CoapRequest request = clientChannel.createRequest(CoapRequestCode.PUT, "/motor", true);
			request.setPayload(new CoapData("on", CoapMediaType.text_plain));
			displayRequest(request);
			clientChannel.sendMessage(request);
	...
	public static void main(String[] args){
		//프레임 열기
		GUI_client gui = new GUI_client("fe80::6434:226a:198:eef6", CoapConstants.COAP_DEFAULT_PORT);
	}
